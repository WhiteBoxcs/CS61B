...
class Game {
    ...
    void tableauToFoundation(final int t) {
        this.apply(new Action() {
            ...
            @Override
            public void act() {
                Pile tableau = Game.this.tableau(t);
                if (tableau.isEmpty()) {
                    throw err("No cards in that pile");
                }
                Pile foundation = Game.this.findFoundation(tableau.top());
                Game.this.checkFoundationAdd(tableau.top(), foundation);
                foundation.move(tableau, 1);
                Game.this.fillFromReserve(tableau);
            }
    ...
    private class TableauToTableauAction implements Action {
        ...
        @Override
        public void act() {
            Pile t0 = Game.this.tableau(this.k0);
            Pile t1 = Game.this.tableau(this.k1);
            if (t0 == t1) {
            ...
            if (t1.isEmpty()) {
                ...
                Game.this.checkTableauAdd(t0.bottom(), t1);
                t1.move(t0);
            }
            Game.this.fillFromReserve(t0);
        }
    ...
    class FoundationToTableau implements Action {
        ...
        @Override
        public void act() {
            Pile foundation = Game.this.foundation(this.f);
            Pile tableau = Game.this.tableau(this.t);
            if (foundation.isEmpty()) {
                throw err("Cannot move from empty pile");
            } else if (tableau.isEmpty()) {
                throw err("Cannot move card to empty tableau");
            }
            Game.this.checkTableauAdd(foundation.top(), tableau);
    ...
    class WasteToTableau implements Action {
        ...
        @Override
        public void act() {
            Pile p = Game.this.tableau(this.kIndex);
    ...
    private void checkTableauAdd(Card card, Pile p) {
        ...
        if (card.rank() == this._base.rank()) {
            ...
            throw err("tableau is built down in alternating colors");
                ...
                && (t.rank() - card.rank() + Card.NUM_RANKS) % Card.NUM_RANKS != 1) {
