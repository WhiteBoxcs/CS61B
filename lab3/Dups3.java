...
public class Dups3 {
    ...
    static List<String> duplicates(List<String> L) {
        ...
        ListIterator<String> compr = L.listIterator();
        while(compr.hasNext()){
        	String x = compr.next();
        	if(result.contains(x))
        		continue;
        	
        	ListIterator<String> dupChecker = L.listIterator(compr.nextIndex());
        	while(dupChecker.hasNext())
        		if(x.equals(dupChecker.next())){
        			result.add(x);
                    break;
        		}
        }
