...
import static db61b.Utils.mkPatn;
...
class Tokenizer {
    ...
    private static final String LITERAL_TEXT = "'(?:[^,'\n\r]*)'?",
            ...
            COMMENT_TEXT = "(?:/\\*.*?\\*/|/\\*.*)";
    /**
     * Matches potential tokens, including valid or unterminated literals,
     * valid or unterminated comments, identifiers, relation symbols (=,
     * <, <=, >=, and !=), end-of-line sequences, or other single characters.
     * The pattern matches a prefix of any string.
     */
    ...
    static final Pattern IDENTIFIER = mkPatn(IDENTIFIER_TEXT),
            LITERAL = mkPatn("'.*"), RELATION = mkPatn("[<>!]?=|[<>]");
    ...
    Tokenizer(Scanner s, PrintStream prompter) {
        ...
        this._k = 0;
        this._mat = Pattern.compile(".").matcher("");
    }

    /**
     * Read the next potential token and add it to _buffer. Filters out
     * comments and ends-of-line. Adds "*EOF*" at the end of input. Throws
     * DBException on encountering a lexical error (such as an unterminated
     * literal).
     */
    private void readToken() {
        while (true) {
            this.prompt();
            String token = this._input.findWithinHorizon(TOKEN_PATN, 0);
            if (token == null) {
                token = "*EOF*";
            ...
            this._buffer.add(token);
            this._continued = !token.equals(";");
            return;
        }
    }
     ...
     */
    private void prompt() {
        if (this._shouldPrompt && this._prompter != null) {
            if (this._continued) {
                this._prompter.print("...");
            } else {
                this._prompter.print("> ");
            }
            this._prompter.flush();
    ...
    String next(Pattern p) {
        if (!this.nextIs(p)) {
            if (this.nextIs("*EOF*")) {
                ...
                throw error("unexpected token: '%s'", this.peek());
            }
        }
        return this.next();
    }
    ...
    boolean nextIs(Pattern p) {
        ...
        return this._mat.usePattern(p).reset(token).matches();
    }
    ...
    String next() {
        if (this._k == this._buffer.size()) {
            this.readToken();
        }
        this._k += 1;
        return this._buffer.get(this._k - 1);
    }

    /** Returns the next token without changing the position of THIS. */
    String peek() {
        while (this._k >= this._buffer.size()) {
            this.readToken();
        }
        return this._buffer.get(this._k);
    }
