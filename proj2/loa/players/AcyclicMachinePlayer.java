...
public class AcyclicMachinePlayer extends MachinePlayer {
    ...
    public Move act(Move input) {
        TreeMap<Double, Move> moves = suggestedMoves(_game.getBoard());
        Entry<Double, Move> bestMove = moves.lastEntry();
        
        Move finalMove;
        if(cyclicMove(bestMove.getValue()) && moves.size() > 1){
        ...
        if(history.size() > SEARCH_HISTORY_DEPTH)
            history.remove();
     ...
     * @param value
     * @return False if only 3 moves are made since there couldn't possibly be a cycle.
     * False if there is no cycle.
     */
    protected boolean cyclicMove(Move value) {
        ArrayList<Move> quickHist = new ArrayList<Move>(history);
        ...
        {
            for(int i = 1; i <  quickHist.size() - endCycle && endCycle - i >= 0; i++){
                if(!quickHist.get(endCycle-i).equals(
                        quickHist.get(quickHist.size()-i)))
                    return false;
            }
